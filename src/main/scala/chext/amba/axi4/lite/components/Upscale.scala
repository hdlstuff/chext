...
import chext.amba.axi4.full.components.helpers.{SteerLeft, SteerRight}
import chext.amba.axi4.full.components.AddressGenerator
...
case class UpscaleConfig(
  ...
  require(axiSlaveCfg.lite, "axiSlaveCfg.lite must be true!")
...
class Upscale(val cfg: UpscaleConfig) extends Module {
  ...
  val s_axi = IO(axi4.lite.Slave(axiSlaveCfg))
  val m_axi = IO(axi4.lite.Master(axiMasterCfg))
