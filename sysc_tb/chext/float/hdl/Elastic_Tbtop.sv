...
module Wrapper(
  ...
  wire [31:0] _qOutput_ram_dataOutB;
  wire        _ctr_io_full;
  wire        qOutput_source_bits_sign = moduleOut_sign;
  wire [7:0]  qOutput_source_bits_exponent = moduleOut_exponent;
  wire [22:0] qOutput_source_bits_mantissa = moduleOut_mantissa;
  wire        qOutput_sink_ready = sink_ready;
  reg  [2:0]  qOutput_enq_ptr_value;
  reg  [2:0]  qOutput_deq_ptr_value;
  reg         qOutput_maybe_full;
  wire        qOutput_ptr_match = qOutput_enq_ptr_value == qOutput_deq_ptr_value;
  wire        qOutput_source_valid;
  wire        qOutput_source_ready;
  wire        qOutput_do_enq = qOutput_source_ready & qOutput_source_valid;
  wire        qOutput_sink_valid;
  wire        ctr_io_decEn = qOutput_sink_ready & qOutput_sink_valid;
  assign qOutput_sink_valid = ~(qOutput_ptr_match & ~qOutput_maybe_full);
  assign qOutput_source_ready = ~(qOutput_ptr_match & qOutput_maybe_full);
  wire [22:0] qOutput_sink_bits_mantissa = _qOutput_ram_dataOutB[22:0];
  wire [7:0]  qOutput_sink_bits_exponent = _qOutput_ram_dataOutB[30:23];
  wire        qOutput_sink_bits_sign = _qOutput_ram_dataOutB[31];
  wire        source_ready_0 = ~_ctr_io_full & source_valid;
  reg         source_valid_r;
  reg         source_valid_r_1;
  reg         source_valid_r_2;
  reg         source_valid_r_3;
  assign qOutput_source_valid = source_valid_r_3;
  always @(posedge clock) begin
    if (reset) begin
      qOutput_enq_ptr_value <= 3'h0;
      qOutput_deq_ptr_value <= 3'h0;
      qOutput_maybe_full <= 1'h0;
    end
    else begin
      if (qOutput_do_enq)
        qOutput_enq_ptr_value <= qOutput_enq_ptr_value + 3'h1;
      if (ctr_io_decEn)
        qOutput_deq_ptr_value <= qOutput_deq_ptr_value + 3'h1;
      if (qOutput_do_enq != ctr_io_decEn)
        qOutput_maybe_full <= qOutput_do_enq;
    end
  ...
  chext_mem_1w1r #(
    ...
    .clock    (clock),
    .addrA    (qOutput_enq_ptr_value),
    .writeEnA (qOutput_do_enq),
    .dataInA
      ({qOutput_source_bits_sign,
        qOutput_source_bits_exponent,
        qOutput_source_bits_mantissa}),
    .addrB    (qOutput_deq_ptr_value),
    .dataOutB (_qOutput_ram_dataOutB)
  );
  assign source_ready = source_ready_0;
  assign sink_bits_sign = qOutput_sink_bits_sign;
  assign sink_bits_exponent = qOutput_sink_bits_exponent;
  assign sink_bits_mantissa = qOutput_sink_bits_mantissa;
  assign sink_valid = qOutput_sink_valid;
...
module Wrapper_1(
  ...
  wire [63:0] _qOutput_ram_dataOutB;
  wire        _ctr_io_full;
  wire        qOutput_source_bits_sign = moduleOut_sign;
  wire [10:0] qOutput_source_bits_exponent = moduleOut_exponent;
  wire [51:0] qOutput_source_bits_mantissa = moduleOut_mantissa;
  wire        qOutput_sink_ready = sink_ready;
  reg  [2:0]  qOutput_enq_ptr_value;
  reg  [2:0]  qOutput_deq_ptr_value;
  reg         qOutput_maybe_full;
  ...
  assign qOutput_sink_valid = ~(qOutput_ptr_match & ~qOutput_maybe_full);
  assign qOutput_source_ready = ~(qOutput_ptr_match & qOutput_maybe_full);
  wire [51:0] qOutput_sink_bits_mantissa = _qOutput_ram_dataOutB[51:0];
  wire [10:0] qOutput_sink_bits_exponent = _qOutput_ram_dataOutB[62:52];
  wire        qOutput_sink_bits_sign = _qOutput_ram_dataOutB[63];
  wire        source_ready_0 = ~_ctr_io_full & source_valid;
  reg         source_valid_r;
  reg         source_valid_r_1;
  reg         source_valid_r_2;
...
module Wrapper_2(
  ...
  reg         source_valid_r_1;
  reg         source_valid_r_2;
  assign qOutput_source_valid = source_valid_r_2;
  always @(posedge clock) begin
    if (reset) begin
      qOutput_enq_ptr_value <= 3'h0;
      qOutput_deq_ptr_value <= 3'h0;
