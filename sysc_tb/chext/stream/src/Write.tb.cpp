#include <Write_Tbtop_DropEmpty.hpp>
#include <Write_Tbtop_KeepAll.hpp>
...
private:
    ...
    void entry() override {
...

        {
            ...
            SC_SPAWN_TO(j) {
                ...
                for (unsigned i = 0; i < totalTasks; ++i) {
                    ...
                    dut.wr_sourceTask.send({ .address = (uint32_t)(sentBeats * 4), .length = length });
            ...
            SC_SPAWN_TO(j) {
                for (unsigned i = 0; i < totalBeats; ++i) {
                    dut.wr_sourceData.send((i << 16) + i);
                }
            };

            SC_SPAWN_TO(j) {
                for (unsigned i = 0; i < totalTasks; ++i) {
                    fmt::print("received: {}, i = {}\n", dut.wr_sinkResult.receive(), i);
                }
            };
            ...
            SC_SPAWN_TO(j) {
                dut.rd_sourceTask.send({ .address = 0, .length = totalBeats });
            };

            SC_SPAWN_TO(j) {
                for (unsigned i = 0; i < totalBeats; ++i) {
                    auto received = dut.rd_sinkResult.receive().data;
...

        {
            ...
            SC_SPAWN_TO(j) {
                dut.rd_sourceTask.send({ .address = 0, .length = sentBeats });
            };
