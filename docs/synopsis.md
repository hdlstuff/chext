...
s_axi.masterProps(Master.ReadBurstShape) = BurstShape(
  ...
  size = BurstShape.validSizes(s_axi.cfg.wData),
  align = 3 // Every transaction starts on an 8-byte boundary.
)
...
s_axi.masterProps(Master.WriteTrafficProfile).outstandingTransactions = 4</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#e7f0ff;color:#0b4f9c;padding:2px 6px;border-radius:4px;">Scala</span> Read and write facts use separate <code>BurstShape</code>, <code>ThreadMode</code>, and placeholder <code>TrafficProfile</code> keys. <code>BurstShape</code> uses maximum beat count <code>len</code>, normalized <code>tpe</code> and <code>size</code> sequences, and scalar <code>align</code> encoded as <code>log2(bytes)</code>. Masters guarantee alignment, slaves require it, and compatibility requires the master value to be at least the slave value. <code>MemoryMap</code> is a slave property re-exported from <code>tracking.values</code>. <code>PropertyValueType[T]</code> matches keys by their runtime value class. Field updates copy and enforce aggregate values; equal re-enforcement is idempotent, while conflicting re-enforcement fails. At root completion, the checker validates Full burst shapes, Full/Lite thread modes, and each interface's memory map. AXI4-Lite burst shapes are <code>Undefined</code>; <code>TrafficProfile</code> is not checked.</td>
    </tr>
...
val memoryMap = demux.genDecoder(
  ...
  numOutstandingWrite = 32
    ...
    <tr id="entry-axi4-full-components-upscale">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI width upscaler. Its private <code>Upscale_Resolver</code> enforces <code>SingleThread</code> on both sides, preserves master <code>len</code>, <code>tpe</code>, <code>size</code>, and address alignment, derives accepted input sizes and alignment from the downstream shape, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally adapts read/write data channels without changing transaction start addresses.</td>
    </tr>
  ...
  simCheckBurst = chext.util.SimulationCheck.Default
    ...
    <tr id="entry-axi4-full-components-downscale">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI width downscaler for ID-free, single-beat input. Its private <code>Downscale_Resolver</code> enforces <code>SingleThread</code>, derives output INCR sizes and beat counts from each input size, preserves address alignment, calculates the reverse accepted input set from downstream capacity, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. It may create an output burst; use <code>Unburst</code> before it for burst-capable input and after it for a single-beat downstream interface.</td>
    </tr>
  ...
  numOutstandingWrite = 32
    ...
    <tr id="entry-axi4-full-components-unburst">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI burst decomposition. Its private <code>Unburst_Resolver</code> enforces <code>SingleThread</code>, generates a one-beat INCR master shape, reduces its alignment guarantee when generated beat addresses require it, derives accepted input sizes and alignment from downstream one-beat support, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally generates per-beat addresses and coordinates response/data channels.</td>
    </tr>
  ...
  numOutstandingWrite = 32
    ...
    <tr id="entry-axi4-full-components-widen">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI data widening helper. Its private <code>Widen_Resolver</code> preserves <code>ThreadMode</code> and address alignment, removes FIXED bursts, forces full-width output transfers, calculates worst-case output beats and the reverse accepted input shape, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally coordinates address, strobe, data, and response handling through channel logic.</td>
    </tr>
  ...
  Module(new axi4f.components.ProtocolConverter(cfg))</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Staged Full AXI protocol conversion using ID demux/serialization, width conversion, unbursting, and ID mux stages. Its resolver exposes the composed master values and reverse-composed slave values at the external boundary. <code>Slave.MemoryMap</code> flows from external <code>m_axi</code> to <code>s_axi</code>; only <code>TrafficProfile</code> remains <code>Incomplete</code>.</td>
    </tr>
