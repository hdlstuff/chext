...
<table style="table-layout:fixed;width:100%;">
  ...
  wUserB = 0
    ...
    <tr id="entry-axi-connects-tracking-properties">
      <td style="vertical-align:top;">AXI4 aggregate tracking properties <a href="#entry-axi-connects-tracking-properties" style="text-decoration:none;" aria-label="Permalink to entry-axi-connects-tracking-properties">#</a><div style="margin-top:6px;font-size:0.92em;"><div><a href="../src/main/scala/chext/amba/axi4/tracking/values/Values.scala" style="text-decoration:none;"><span style="background:#e7f0ff;color:#0b4f9c;padding:2px 6px;border-radius:4px;">Scala</span></a></div><div><a href="../src/main/scala/chext/amba/axi4/tracking/Properties.scala" style="text-decoration:none;"><span style="background:#e7f0ff;color:#0b4f9c;padding:2px 6px;border-radius:4px;">Scala</span></a></div><div><a href="../src/main/scala/chext/amba/axi4/tracking/CompatibilityChecker.scala" style="text-decoration:none;"><span style="background:#e7f0ff;color:#0b4f9c;padding:2px 6px;border-radius:4px;">Scala</span></a></div><div><a href="../src/test/scala/chext/amba/axi4/tracking/Properties.test.scala" style="text-decoration:none;"><span style="background:#edf2ff;color:#364fc7;padding:2px 6px;border-radius:4px;">Scala TB</span></a></div></div></td>
      <td style="vertical-align:top;">Declare generated and accepted traffic facts:<br><pre style="white-space:pre-wrap;overflow-wrap:anywhere;margin:6px 0 0;padding:8px;background:#f6f8fa;border:1px solid #d0d7de;border-radius:6px;line-height:1.45;"><code class="language-scala">import chext.amba.axi4.tracking.properties.{Master, Slave}
...
s_axi.masterProps(Master.ReadBurstShape) = BurstShape(
  ...
  burstSizes = BurstShape.validSizes(s_axi.cfg.wData)
)
s_axi.masterProps(Master.ReadThreadMode) = ThreadMode.SingleThread
m_axi.slaveProps(Slave.MemoryMap) = MemoryMap(size = 0x1000)

// Field-oriented updates also create an enforced aggregate value.
s_axi.masterProps(Master.WriteTrafficProfile).outstandingTransactions = 4</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#e7f0ff;color:#0b4f9c;padding:2px 6px;border-radius:4px;">Scala</span> Read and write facts use separate <code>BurstShape</code>, <code>ThreadMode</code>, and placeholder <code>TrafficProfile</code> keys; <code>MemoryMap</code> is a slave property and is re-exported from <code>tracking.values</code>. Property keys retain a runtime value-class token for <code>PropertyValueType[T]</code> resolver matching. Aggregate field updates copy and enforce their value, equal complete re-enforcement is idempotent, and conflicting complete re-enforcement throws. Binding marks every AXI4-Lite burst-shape property <code>Undefined</code>; at root completion the checker validates enabled Full burst pairs, Full/Lite thread pairs, and each interface's memory map. <code>TrafficProfile</code> is intentionally not checked yet.</td>
    </tr>
...
val m_axil = IO(axi4l.Master(axiCfg))</code></pre></td>
    ...
    <tr id="entry-axi-connects-lite-single-connect">
      ...
      <td style="vertical-align:top;">Creates <code>axi4l.Connect</code>; <span style="background:#f1f3f4;color:#3c4043;padding:2px 6px;border-radius:4px;">UniquePrefix: axi4lConnect</span>; graph <code>tpe</code> is <code>Axi4l_Connect</code>. Exposes each available master/slave AXI channel as a boundary interface; channel-level child <code>Connect</code> components retain operational ownership. Binding leaves all Lite <code>BurstShape</code> properties <code>Undefined</code>; a private <code>Connect_Resolver(owner: Connect)(implicit sourceInfo: SourceInfo)</code> transparently forwards master <code>ThreadMode</code> downstream and slave <code>ThreadMode</code> / <code>MemoryMap</code> upstream, while direct <code>TrafficProfile</code> requests terminate as <code>Incomplete</code>.</td>
    </tr>
...
AXI4 full components are Chisel modules. They are not Chext components themselves; graph content comes from the elastic components they instantiate internally. Every module that declares tracked AXI ports creates a neighboring private <code>X_Resolver(owner: X)(implicit sourceInfo: SourceInfo)</code>. The standard read/write catalogs contain aggregate <code>BurstShape</code>, <code>ThreadMode</code>, and placeholder <code>TrafficProfile</code> keys; the slave catalog also contains <code>MemoryMap</code>, re-exported from <code>chext.amba.axi4.tracking.values</code>. Typed runtime <code>PropertyValueType[T]</code> extractors let resolver policies match an aggregate family while retaining its concrete value type for forwarding, transformations, and calculation; singleton-key matches are reserved for truly key-specific behavior. <code>bindMaster</code> and <code>bindSlave</code> register participating interfaces, and the inherited <code>MasterRequests</code> and <code>SlaveRequests</code> extractors match typed requests across all corresponding bindings; their nested <code>withInterface</code> extractors also expose the selected endpoint when needed. Resolver trace names default from the implementation class name: <code>_Resolver</code> is normalized to <code>Resolver</code>, and the kind is its kebab-case base name. Field assignment on aggregate properties produces an authoritative enforced value, and conflicting complete re-enforcement is an error. Binding eagerly marks standard properties for directions disabled by <code>axi4.Config</code> as <code>Undefined</code>, and also marks every AXI4-Lite burst shape <code>Undefined</code>. After root construction, tracking resolves and validates burst shape on Full interfaces, thread mode on Full/Lite interfaces, and memory maps on every registered interface, reporting all local incompatibilities with both resolution traces. <code>TrafficProfile</code> is deliberately excluded from that first pass and direct requests resolve as <code>Incomplete</code>. Fan-in/fan-out behavior stays in owner-specific resolvers. For the detailed guide and examples for every full AXI component/helper, see [axi4-full.md](axi4-full.md).
...
<table style="table-layout:fixed;width:100%;">
  ...
  arbiterPolicy = e.Chooser.rr
    ...
    <tr id="entry-axi4-full-components-demux">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI fan-out by address/routing selection. Its private <code>Demux_Resolver</code> forwards each upstream master <code>BurstShape</code> and <code>ThreadMode</code> unchanged to every output. Downstream slave burst/thread capabilities remain separate, so their synthetic aggregate at <code>s_axi</code> is <code>DontCare</code> and compatibility is checked at the outputs. An arbitrary <code>decodeFn</code> cannot describe a combined address map, so <code>Slave.MemoryMap</code> remains <code>Incomplete</code> and strict map checking rejects the unresolved topology. <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally uses channel-level elastic wiring around <code>ar</code>, <code>r</code>, <code>aw</code>, <code>w</code>, and <code>b</code>.</td>
    </tr>
...
val memoryMap = demux.genDecoder(
  ...
  allocationScheme = MemoryMap.AllocationScheme.AlignedPacked
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Resolves each mapped master interface's <code>Slave.MemoryMap</code>; AXI <code>Connect</code> and tracked <code>Buffer</code> component resolvers allow those properties to propagate through downstream connections and buffered links. <code>Resolver.resolve</code> returns a <code>Resolution</code> containing the terminal result and trace after recursively traversing dependencies depth-first, with the depth bounded by <code>maxStackSize</code>. Resolver candidates prefer the shallowest owner, with the most recently registered candidate winning at equal depth. It optionally reorders the maps with a complete permutation of non-error interfaces and calls <code>MemoryMap.aggregate</code> with <code>AlignedPacked</code>, <code>AlignedLargest</code>, or <code>Tight</code> allocation. <code>memoryMapPath</code> names a generated map when DemuxMm instances are composed hierarchically. Each decoder subtracts the selected child map's offset from forwarded <code>ARADDR</code> and <code>AWADDR</code>, so the next hierarchy level receives a local address. Maps and segments carry absolute slash-separated origins. Optional <code>captureResolutionTrace</code> records typed <code>interfaceFrom</code>, <code>interfaceTo</code>, <code>kind</code>, <code>resolver</code>, and <code>resolverPath</code> steps in map arguments. <code>errorSlave = Some(index)</code> reserves that <code>m_axi</code> interface for addresses outside all mapped segments, so its map may be <code>Undefined</code>. Without an error slave, misses select the first interface in address order. Every map has an explicit declared size; validation rejects out-of-bounds, overlapping, and address-width-exceeding layouts. A private <code>DemuxMm_Resolver</code> publishes the generated aggregate map after <code>genDecoder()</code>, forwards upstream master burst/thread facts to every output, keeps downstream burst/thread capabilities separate with <code>DontCare</code>, and leaves <code>TrafficProfile</code> <code>Incomplete</code>.</td>
    </tr>
  ...
  arbiterPolicy = e.Chooser.rr
    ...
    <tr id="entry-axi4-full-components-mux">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI fan-in. Its private <code>Mux_Resolver</code> forwards the common downstream slave <code>BurstShape</code>, <code>ThreadMode</code>, and <code>MemoryMap</code> to every input. Master <code>BurstShape</code> at the common output is <code>DontCare</code>; with one input its master <code>ThreadMode</code> is forwarded, while multiple inputs conservatively produce <code>Unconstrained</code>. <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally creates arbitration and channel-level elastic wiring for read and write paths.</td>
    </tr>
  ...
  arbiterPolicy = e.Chooser.rr
    ...
    <tr id="entry-axi4-full-components-id-demux">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> ID-based full AXI fan-out. Its private <code>IdDemux_Resolver</code> forwards upstream master <code>BurstShape</code> and <code>ThreadMode</code> to each ID-selected output. Slave <code>BurstShape</code>, <code>ThreadMode</code>, and <code>MemoryMap</code> are <code>DontCare</code> at the common input so downstream capabilities remain separate; <code>TrafficProfile</code> is <code>Incomplete</code>. Internally expands to elastic channel components and rewrites ID bits for routing.</td>
    </tr>
  ...
  arbiterPolicy = e.Chooser.rr
    ...
    <tr id="entry-axi4-full-components-id-mux">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> ID-aware full AXI fan-in. Its private <code>IdMux_Resolver</code> forwards downstream slave <code>BurstShape</code>, <code>ThreadMode</code>, and <code>MemoryMap</code> to every input. Output master <code>BurstShape</code> is <code>DontCare</code>; master <code>ThreadMode</code> is forwarded for one input and is <code>Unconstrained</code> for multiple prefixed-ID inputs. <code>TrafficProfile</code> is <code>Incomplete</code>. Internally uses ID-based response routing and channel-level elastic components.</td>
    </tr>
  ...
  Module(new axi4f.components.IdSerialize(cfg))</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI ID serialization. Its private <code>IdSerialize_Resolver</code> transparently forwards burst shapes and the memory map, accepts <code>Unconstrained</code> input threads, and generates <code>SingleThread</code> output traffic. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally limits outstanding work and routes response channels through elastic control logic.</td>
    </tr>
  ...
  Module(new axi4f.components.IdParallelize(cfg))</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI ID parallelization. Its private <code>IdParallelize_Resolver</code> transparently forwards burst shapes and the memory map, accepts <code>SingleThread</code> input traffic, and generates <code>UniqueThreads</code> at the output. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally distributes requests and rejoins responses with elastic channel logic.</td>
    </tr>
  ...
  numOutstandingWrite = 32
    ...
    <tr id="entry-axi4-full-components-upscale">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI width upscaler. Its private <code>Upscale_Resolver</code> enforces <code>SingleThread</code> on both sides, preserves master beats/types/sizes while recalculating narrow support for the wider bus, derives accepted input sizes from the downstream shape, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally adapts read/write data channels and keeps address/control channels aligned.</td>
    </tr>
  ...
  simCheckBurst = chext.util.SimulationCheck.Default
      ...
      <td style="vertical-align:top;"><span style="background:#e6f4ea;color:#0d652d;padding:2px 6px;border-radius:4px;">Config</span> Configuration for <code>Downscale</code>; master data width must be narrower than the slave data width. The input must have no transaction IDs (<code>wId == 0</code>) and every request must be single-beat (<code>ARLEN == 0</code>, <code>AWLEN == 0</code>). <code>simCheckBurst</code> controls optional simulation checks for the single-beat precondition.</td>
    </tr>
    <tr id="entry-axi4-full-components-downscale">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI width downscaler for ID-free, single-beat input. Its private <code>Downscale_Resolver</code> enforces <code>SingleThread</code>, derives output INCR sizes and beat counts from each input size, calculates the reverse accepted input set from downstream capacity, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. It may create an output burst; use <code>Unburst</code> before it for burst-capable input and after it for a single-beat downstream interface.</td>
    </tr>
  ...
  numOutstandingWrite = 32
    ...
    <tr id="entry-axi4-full-components-unburst">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI burst decomposition. Its private <code>Unburst_Resolver</code> enforces <code>SingleThread</code>, generates a one-beat INCR master shape, derives accepted input sizes from downstream one-beat support, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally generates per-beat addresses and coordinates response/data channels.</td>
    </tr>
  ...
  numOutstandingWrite = 32
    ...
    <tr id="entry-axi4-full-components-widen">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI data widening helper. Its private <code>Widen_Resolver</code> preserves <code>ThreadMode</code>, removes FIXED bursts, forces full-width output transfers, calculates worst-case aligned output beats and the reverse accepted input shape, and forwards <code>Slave.MemoryMap</code>. Only <code>TrafficProfile</code> remains <code>Incomplete</code>. Internally coordinates address, strobe, data, and response handling through channel logic.</td>
    </tr>
  ...
  Module(new axi4f.components.CreditBuffer(cfg))</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Full AXI credit-style response buffering around read/write channels. <code>CreditBuffer_Resolver</code> transparently forwards master/slave <code>BurstShape</code> and <code>ThreadMode</code> plus <code>Slave.MemoryMap</code>; <code>TrafficProfile</code> remains <code>Incomplete</code> until concurrency and latency semantics are implemented.</td>
    </tr>
  ...
  Module(new axi4f.components.ProtocolConverter(cfg))</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> Staged full AXI protocol conversion. It composes ID demux/serialization, width conversion, unbursting, and ID mux stages. <code>ProtocolConverter_Resolver</code> exposes the composed master and reverse slave aggregate values across the external boundary, including the final manually wired bridge; <code>Slave.MemoryMap</code> reaches <code>s_axi</code> from external <code>m_axi</code>, while only <code>TrafficProfile</code> is deliberately <code>Incomplete</code>.</td>
    </tr>
...
liteConverter0.m_axil :=&gt; memController.s_axil</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> ID-free Full-to-Lite conversion with an authoritative Full-input burst shape and <code>SingleThread</code> mode on both sides; Lite burst properties remain <code>Undefined</code>. Its bridge resolver publishes the protocol-implied one-beat shape only on the internal Full interface, and composes remaining facts through the unburst/width stages, including forwarding <code>Slave.MemoryMap</code> from <code>m_axil</code> to <code>s_axi</code>; <code>TrafficProfile</code> remains <code>Incomplete</code>. Input unbursting always precedes inferred width conversion. Upscaling steers data and write strobes without adding beats; downscaling is followed by a second unburst stage.</td>
    </tr>
...
AXI4-Lite uses <code>ThreadMode</code>, <code>TrafficProfile</code>, and <code>MemoryMap</code>; every master/slave <code>BurstShape</code> property is eagerly <code>Undefined</code> when a resolver binds the interface because Lite exposes no burst-shape signals. Only <code>SingleTransaction</code> and <code>SingleThread</code> are valid Lite thread modes; <code>UniqueThreads</code> or <code>Unconstrained</code> on either side is an elaboration error with resolution provenance. Declared interfaces participate in root-level thread-mode and memory-map validation, while burst compatibility is checked only for Full AXI. <code>TrafficProfile</code> is excluded from the initial pass and direct requests remain <code>Incomplete</code>. For the detailed guide and examples for every Lite AXI component/helper, see [axi4-lite.md](axi4-lite.md).
...
val idleMaster = Module(new axi4l.components.IdleMaster(liteCfg))</code></pre></td>
  ...
  masterBuffers = axi4.BufferConfig.all(0)
    ...
    <tr id="entry-axi4-lite-components-demux">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> AXI4-Lite fan-out for address/data/control channels. Its private <code>Demux_Resolver</code> forwards upstream master <code>ThreadMode</code> to every output, keeps downstream slave thread capabilities separate with <code>DontCare</code> at <code>s_axil</code>, and leaves the arbitrary-decode <code>MemoryMap</code> plus every <code>TrafficProfile</code> request <code>Incomplete</code>. Lite burst shapes remain <code>Undefined</code>.</td>
    </tr>
  ...
  arbiterPolicy = e.Chooser.rr
    ...
    <tr id="entry-axi4-lite-components-mux">
      ...
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> AXI4-Lite fan-in with channel-level elastic arbitration and routing. Its private <code>Mux_Resolver</code> forwards slave <code>ThreadMode</code> and <code>MemoryMap</code> from <code>m_axil</code> to each input. Output master <code>ThreadMode</code> is <code>SingleThread</code>, every Lite burst shape is <code>Undefined</code>, and <code>TrafficProfile</code> remains <code>Incomplete</code>.</td>
    </tr>
  ...
  Module(new axi4l.components.CreditBuffer(cfg))</code></pre></td>
      <td style="vertical-align:top;"><span style="background:#fff4d6;color:#8a5a00;padding:2px 6px;border-radius:4px;">Module</span> AXI4-Lite response buffering with credit-style control around read/write channels. Its resolver transparently forwards master/slave <code>ThreadMode</code> plus <code>Slave.MemoryMap</code>; Lite burst shapes remain <code>Undefined</code>, and <code>TrafficProfile</code> remains <code>Incomplete</code>.</td>
    </tr>
